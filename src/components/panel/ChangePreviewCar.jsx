    ...
    <ModalBackdrop open>
      ...
      <section className='grid grid-rows-3 grid-flow-col overflow-x-auto p-1 gap-2 md:place-content-center'>
        {car.image.map((image, index) => (
          <img onClick={() => setSelectedImage(image)} key={index} className={`w-44 max-w-fit cursor-pointer hover:opacity-60 select-none max-h-44 rounded ${selectedImage === image && selectedClass} h-auto object-contain`} src={image} alt={car.name} />
        ))}
      </section>
